...
MODULE_DESCRIPTION("Flexibilis Redundant Switch (FRS/FES/RS) driver");
...
static int __init flx_frs_init(void)
{
    ...
    // Write registers with inter-device dependencies
                                                 ...
                                                 struct flx_frs_cfg *frs_cfg)
{
    ...
    if (!frs_cfg_pdata) {
        ...
        // Switch type, may be refined later from registers.
        if (of_device_is_compatible(dev->of_node, "flx,frs"))
            dp->type = FLX_FRS_TYPE_FRS;
        else if (of_device_is_compatible(dev->of_node, "flx,fes"))
            dp->type = FLX_FRS_TYPE_FES;
        else if (of_device_is_compatible(dev->of_node, "flx,rs"))
            dp->type = FLX_FRS_TYPE_RS;
        else if (of_device_is_compatible(dev->of_node, "flx,ds"))
            dp->type = FLX_FRS_TYPE_DS;

        if (!irq_res) {
            dev_err(dp->this_dev, "No IRQ defined\n");
            return NULL;
        }
        ...
        for_each_child_of_node(dev->of_node, child_n) {
            ...
            if (of_property_read_u32(child_n, "medium_type", &ivalue)) {
                ...
                switch (ivalue) {
                case FLX_FRS_MEDIUM_CPU:
                    // for compatibility
                    port_cfg->medium_type = FLX_FRS_MEDIUM_NOPHY;
                    port_cfg->flags |= FLX_FRS_PORT_CPU;
                    break;
                case FLX_FRS_MEDIUM_IE:
                    // for compatibility
                    port_cfg->medium_type = FLX_FRS_MEDIUM_NOPHY;
            ...
            if (!reg ||
                ...
                dev_printk(KERN_DEBUG, dp->this_dev,
                           "port %i reg 0x%x adapter 0x%x\n",
                           port_num, port_cfg->baseaddr,
                           port_cfg->adapter_baseaddr);
            }
                              ...
                              struct flx_frs_cfg *frs_cfg)
{
    ...
    for (i = 0; i < frs_cfg->num_of_ports; i++) {
        ...
        pp->fwd_state = FRS_PORT_FWD_STATE_AUTO;
        pp->rx_delay = 0;
        pp->tx_delay = 0;
        pp->p2p_delay = 0;
...
static int flx_frs_init_registers(struct flx_frs_dev_priv *dp)
{
    ...
    // Detect actual switch type
    if (dp->type == FLX_FRS_TYPE_FRS || dp->type == FLX_FRS_TYPE_DS) {
        data = flx_frs_read_switch_reg(dp, FRS_REG_CONFIG_ID);
        if (data == FRS_CONFIG_ID_DS)
            dp->type = FLX_FRS_TYPE_DS;
        else
            dp->type = FLX_FRS_TYPE_FRS;
    }
...
static void flx_frs_trailer_fixup(struct flx_frs_dev_priv *dp)
{
    ...
    for (i = 0; i < ARRAY_SIZE(drv->dev_priv); i++) {
	    ...
	    if (dp_tmp->type == FLX_FRS_TYPE_DS) {
		dev_printk(KERN_DEBUG, dp_tmp->this_dev,
			   "Setting trailer MACsec bit on\n");
		// Most significant bit in DS indicates MACsec
		port->port_mask |= (1u << (dp_tmp->trailer_len * 8 - 1));
	    }

            dev_printk(KERN_DEBUG, dp_tmp->this_dev,
                       "  Port %u send trailer 0x%x\n",
                       port->port_num, port->port_mask);
            }
    }
