...
/**
 * Switch types that need to be handled slightly differently.
 */
enum flx_frs_type {
    FLX_FRS_TYPE_FRS,
    FLX_FRS_TYPE_FES,
...
struct flx_frs_port_priv {
    ...
    enum frs_port_fwd_state_val fwd_state; ///< Forwarding state
...
struct flx_frs_dev_priv {
    ...
    uint16_t cpu_port_mask;             ///< port mask of CPU port
    ...
    struct mutex mac_table_lock;        ///< synchronize MAC table access
    struct mutex smac_table_lock;       ///< synchronize SMAC table access

    enum flx_frs_type type;             ///< switch type
