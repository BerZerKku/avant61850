 ...
 * @return Link mode.
 */
enum link_mode get_link_mode_from_phy(struct phy_device *phy)
{
    int speed = phy->speed;
 ...
 * @return Link mode.
 */
static enum link_mode get_link_mode_from_adapter(struct flx_frs_port_priv *pp)
{
                                ...
                                struct ethtool_cmd *cmd)
{
    ...
    case FLX_FRS_MEDIUM_PHY:
        ...
        if (cmd->autoneg == AUTONEG_ENABLE) {
            ...
            if (ext_phy->phydev && ext_phy->phydev->state != PHY_DOWN) {
                // PHY present and used
                link_mode = get_link_mode_from_phy(ext_phy->phydev);
            }
            else if (sfp_phy->phydev && sfp_phy->phydev->state != PHY_DOWN) {
                // SFP PHY present and used
                link_mode = get_link_mode_from_phy(sfp_phy->phydev);
            }
            else {
                link_mode = get_link_mode_from_adapter(pp);
            }
        } else {
            // Forced
        ...
        if (ret == 0 &&
            ...
            if (ext_phy->phydev && ext_phy->phydev->state != PHY_DOWN) {
                cmd->phy_address = ext_phy->phydev->addr;
                ret = phy_ethtool_sset(ext_phy->phydev, cmd);
            }
            if (ret == 0 &&
                sfp_phy->phydev && sfp_phy->phydev->state != PHY_DOWN) {
                cmd->phy_address = sfp_phy->phydev->addr;
...
static const struct flx_frs_stat flx_frs_gstrings_stats[] = {
    ...
    [FRS_CNT_RX_POLICED] = {"rx_policed", PORT_REG_RX_POLICED_L},
    [FRS_CNT_RX_MACSEC_UNTAGGED] = {"rx_macsec_untagged", PORT_REG_RX_MACSEC_UNTAGGED_L},
    [FRS_CNT_RX_MACSEC_NOTSUPP] = {"rx_macsec_notsupp", PORT_REG_RX_MACSEC_NOTSUPP_L},
    [FRS_CNT_RX_MACSEC_UNKNOWNSCI] = {"rx_macsec_unknownsci", PORT_REG_RX_MACSEC_UNKNOWNSCI_L},
    [FRS_CNT_RX_MACSEC_NOTVALID] = {"rx_macsec_notvalid", PORT_REG_RX_MACSEC_NOTVALID_L},
    [FRS_CNT_RX_MACSEC_LATE] = {"rx_macsec_late", PORT_REG_RX_MACSEC_LATE_L},
