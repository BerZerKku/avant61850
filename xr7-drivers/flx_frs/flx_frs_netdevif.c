...
static rx_handler_result_t flx_frs_handle_frame(struct sk_buff **pskb)
{
    ...
    if (port) {
        ...
        // not found
        dev_err(dp->this_dev, "Port not found for frame\n");
    }
        ...
        struct sk_buff *rx_frame)
{
    ...
    dev_resolve_dbg(dp->this_dev, "Trailer 0x%x length %i skb length %i"
                    " eth_hdr %u mac_header %u proto 0x%x\n",
                    trailer, dp->trailer_len, rx_frame->len,
                    (unsigned char *)eth - rx_frame->head,
                    skb_mac_header(rx_frame) - rx_frame->head,
    ...
    if (dp_recv->port[port_num] &&
        dp_recv->port[port_num]->flags & FLX_FRS_PORT_INDEPENDENT) {
        dev_resolve_dbg(dp->this_dev,
    ...
    if (!port) {
        dev_dbg(dp->this_dev, "Device %i port %i is not active\n",
                dp_recv->dev_num, port_num);
        return NULL;
    }

    dev_resolve_dbg(dp->this_dev, "resolved to device %i port %i\n",
                    dp_recv->dev_num, port->port_num);
