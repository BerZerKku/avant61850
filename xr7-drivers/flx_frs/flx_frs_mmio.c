                                       ...
                                       struct flx_frs_cfg *frs_cfg)
{
    ...
    dev_info(dp->this_dev, "Setup device for memory mapped access\n");
    ...
    for (i = 0; i < ARRAY_SIZE(dp->port); i++) {
        ...
        dev_printk(KERN_DEBUG, dp->this_dev,
                   "Port %i uses memory mapped access: 0x%llx/0x%x -> %p\n",
                   pp->port_num, (unsigned long long int) port_base,
                   FLX_FRS_PORT_IOSIZE, pp->regs.ioaddr);
        ...
        dev_printk(KERN_DEBUG, dp->this_dev,
                   ...
                   " 0x%llx/0x%x -> %p\n",
                   pp->port_num, (unsigned long long int) adapter_base,
                   FLX_FRS_ADAPTER_IOSIZE, pp->adapter.regs.ioaddr);
    }
